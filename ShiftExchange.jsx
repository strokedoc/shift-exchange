...
const STORAGE_KEY = "shiftexchange_v5";
...
const NAMES = Array.from({length:12}, (_,i) => `Physician ${i+1}`);
...
export default function App() {
  ...
  const push = async s => {
    setSaving(true);
    try { await window.storage.set(STORAGE_KEY, JSON.stringify(s), true); setState(s); return true; }
  ...
  useEffect(() => {
    (async () => {
      let d = await pull();
      if (!d) { d = fresh(); await window.storage.set(STORAGE_KEY, JSON.stringify(d), true).catch(()=>{}); }
      ...
      <header className="sticky top-0 z-20 bg-white border-b border-slate-200 shadow-sm">
        <div className="max-w-md mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            ...
            {/* Admin gear — always visible in header */}
